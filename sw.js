// TransPgso SW v1782673201
const APP_VERSION = '1782673201';
...
self.addEventListener('fetch', e => {
  ...
  if(esIndex) {
    e.respondWith(fetch(url.pathname.split('?')[0]+'?_nocache='+APP_VERSION,{cache:'no-store'}).catch(()=>fetch(e.request,{cache:'no-store'})));
    return;
  }
