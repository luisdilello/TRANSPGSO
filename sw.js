// TransPgso SW v1783292666
const APP_VERSION = '1783292666';
...
self.addEventListener('notificationclick', e => {
  ...
  const url = (e.notification.data && e.notification.data.url) || '/TRANSPGSO/';
  e.waitUntil(
    self.clients.matchAll({type:'window'}).then(clientsArr => {
      for (const client of clientsArr) { if (client.url.includes('TRANSPGSO') && 'focus' in client) return client.focus(); }
