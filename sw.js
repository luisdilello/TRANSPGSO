// TransPgso SW v1782529783
const APP_VERSION = '1782529783';
...
self.addEventListener('fetch', e => {
  ...
  // Solo interceptar index.html — dejar pasar APIs sin modificar
  ...
  if(esIndex) {
    e.respondWith(fetch(e.request.url.split('?')[0]+'?_nocache='+APP_VERSION,{cache:'no-store'}).catch(()=>fetch(e.request,{cache:'no-store'})));
    return;
  }
  if(url.pathname.endsWith('sw.js')) { e.respondWith(fetch(e.request,{cache:'no-store'})); return; }
  // Todo lo demás (APIs, Workers, Supabase): pasar sin modificar
