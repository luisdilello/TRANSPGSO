...
var Badge=window.__app.Badge, COMUNAS_RM_LIST=window.__app.COMUNAS_RM_LIST, KpiBar=window.__app.KpiBar, Modal=window.__app.Modal, PestanasMensajero=window.__app.PestanasMensajero, db=window.__app.db, enviarNotificacionPush=window.__app.enviarNotificacionPush, fmt=window.__app.fmt;
function GestionMensajeros(_ref15){let mensajerosProp=_ref15.mensajeros,setMensajeros=_ref15.setMensajeros,mensajerosDia=_ref15.mensajerosDia,toast=_ref15.toast,esAdmin=_ref15.esAdmin;
  ...
  // idsConUsuario/nombresConUsuario: qué mensajeros están vinculados a una cuenta en
  // Gestión de Usuarios. Si lo están, el nombre se administra allá (no acá) — ver tieneUsuario().
  const _idsCU=useMemo(()=>new Set((usuariosRider||[]).map(u=>u.mensajero_id).filter(Boolean)),[usuariosRider]);
  const _nomCU=useMemo(()=>new Set((usuariosRider||[]).map(u=>(u.nombre||'').trim().toUpperCase())),[usuariosRider]);
  function tieneUsuario(m){
    if(usuariosRider===null)return false;
    return _idsCU.has(m.id)||_nomCU.has((m.nombre||'').trim().toUpperCase());
  }
  const mensajeros=useMemo(()=>{
    ...
    return mensajerosProp.filter(m=>_idsCU.has(m.id)||_nomCU.has((m.nombre||'').trim().toUpperCase()));
  },[mensajerosProp,usuariosRider,_idsCU,_nomCU]);
...
const filtrados=mensajeros.filter(m=>m.nombre.toLowerCase().includes(search.toLowerCase())&&(filtro==='todos'||filtro==='activos'&&m.activo||filtro==='pausados'&&!m.activo)).sort((a,b)=>(a.nombre||'').localeCompare(b.nombre||'','es'));const normNombre=n=>(n||'').toUpperCase().replace(/,/g,'').replace(/\s+/g,' ').trim();const statsMap={};mensajerosDia.forEach(m=>{if(m.total>0)statsMap[normNombre(m.nombre)]={ef:m.entregados/m.total,total:m.total,entregados:m.entregados};});async function toggleEstado(id){const m=mensajeros.find(x=>x.id===id);const nuevoActivo=!m.activo;setMensajeros(prev=>prev.map(x=>x.id===id?{...x,activo:nuevoActivo}:x));toast(`${m.nombre} ${nuevoActivo?'activado':'pausado'}`);try{const{error:errUpd}=await db.from('mensajeros').update({activo:nuevoActivo}).eq('id',id);if(errUpd)toast('⚠ Error guardando estado: '+errUpd.message);}catch(e){toast('⚠ '+e.message);}}async function save(){
  ...
  if(modal==='add'){
    try{
      const{data:inserted,error:errIns}=await db.from('mensajeros').insert({nombre:nombreFinal,activo:true,tarifa:+form.tarifa,tarifa_retiro:+form.tarifaRetiro||500,ver_ganancias:false,colecta_habilitada:false}).select().single();
    ...
    try{
      const {error:errUpd}=await db.from('mensajeros').update({nombre:nombreFinal,tarifa:+form.tarifa,tarifa_retiro:+form.tarifaRetiro||500}).eq('id',modal);
      if(errUpd)toast('⚠ Error: '+errUpd.message);
      else toast('✓ '+nombreFinal+' · $'+Number(form.tarifa).toLocaleString('es-CL')+'/entrega');
    }catch(e){toast('⚠ '+e.message);}
  }
  setModal(null);
}async function toggleRutaIA(id){const m=mensajeros.find(x=>x.id===id);const nuevo=!m.optimizacionRutaActiva;setMensajeros(prev=>prev.map(x=>x.id===id?{...x,optimizacionRutaActiva:nuevo}:x));toast(`Ruta ${nuevo?'activada':'desactivada'} para ${m.nombre}`);try{const{error:errUpd}=await db.from('mensajeros').update({optimizacion_ruta_activa:nuevo}).eq('id',id);if(errUpd)toast('⚠ Error guardando: '+errUpd.message);}catch(e){toast('⚠ '+e.message);}}async function toggleVerGanancias(id){const m=mensajeros.find(x=>x.id===id);const nuevo=!m.verGanancias;setMensajeros(prev=>prev.map(x=>x.id===id?{...x,verGanancias:nuevo}:x));toast(`Ganancias ${nuevo?'visibles':'ocultas'} para ${m.nombre}`);try{const{error:errUpd}=await db.from('mensajeros').update({ver_ganancias:nuevo}).eq('id',id);if(errUpd)toast('⚠ Error guardando: '+errUpd.message);}catch(e){toast('⚠ '+e.message);}}async function toggleColectaHabilitada(id){const m=mensajeros.find(x=>x.id===id);const nuevo=!m.colectaHabilitada;setMensajeros(prev=>prev.map(x=>x.id===id?{...x,colectaHabilitada:nuevo}:x));toast(`Colecta ${nuevo?'activada':'desactivada'} para ${m.nombre}`);try{const{error:errUpd}=await db.from('mensajeros').update({colecta_habilitada:nuevo}).eq('id',id);if(errUpd)toast('⚠ Error guardando: '+errUpd.message);}catch(e){toast('⚠ '+e.message);}}async function toggleGananciasTodos(){const nuevo=!mensajeros.every(m=>m.verGanancias);const ids=mensajeros.map(m=>m.id);setMensajeros(prev=>prev.map(x=>ids.includes(x.id)?{...x,verGanancias:nuevo}:x));toast(nuevo?'💰 Ganancias visibles para todos los mensajeros':'🙈 Ganancias ocultas para todos los mensajeros');try{const{error:errUpd}=await db.from('mensajeros').update({ver_ganancias:nuevo}).in('id',ids);if(errUpd)toast('⚠ Error guardando: '+errUpd.message);}catch(e){toast('⚠ '+e.message);}}async function del(id){setMensajeros(prev=>prev.filter(m=>m.id!==id));toast('Mensajero eliminado');setConfirmDel(null);try{const{error:errDel}=await db.from('mensajeros').delete().eq('id',id);if(errDel)toast('⚠ No se pudo eliminar en la base: '+errDel.message);}catch(e){toast('⚠ '+e.message);}}return/*#__PURE__*/React.createElement("div",null,sinUsuario.length>0&&esAdmin&&/*#__PURE__*/React.createElement("div",{style:{background:'rgba(200,168,75,0.1)',border:'1px solid var(--gold-border)',borderRadius:10,padding:'12px 16px',marginBottom:14,fontSize:13,color:'var(--text)'}},"⚠ ",/*#__PURE__*/React.createElement("strong",null,sinUsuario.length)," mensajero",sinUsuario.length!==1?'s':''," no aparece",sinUsuario.length===1?'':'n'," aquí porque no tiene",sinUsuario.length===1?'':'n'," cuenta en Usuarios: ",sinUsuario.map(m=>m.nombre.replace(/,\s*/g,' ')).join(', '),". Crea su cuenta en Gestión de Usuarios (rol Rider) para que vuelva a aparecer."),/*#__PURE__*/React.createElement("div",{className:"section-head"},/*#__PURE__*/React.createElement("div",{className:"section-title"},"Gesti\xF3n ",/*#__PURE__*/React.createElement("span",null,"Mensajeros")),/*#__PURE__*/React.createElement("div",{style:{display:'flex',gap:8}},esAdmin&&/*#__PURE__*/React.createElement("button",{className:"btn-secondary",onClick:buscarDuplicados,disabled:buscandoDup},buscandoDup?'Buscando...':'Revisar Duplicados'),esAdmin&&/*#__PURE__*/React.createElement("button",{className:"btn-secondary",onClick:()=>setNotifTarget('all')},"📢 Aviso a Todos"),esAdmin&&/*#__PURE__*/React.createElement("button",{className:"btn-secondary",onClick:toggleGananciasTodos,title:"Mostrar u ocultar la pesta\xF1a Ganancias en la app de todos los mensajeros"},mensajeros.every(m=>m.verGanancias)?'🙈 Ocultar Ganancias a Todos':'💰 Mostrar Ganancias a Todos'))),/*#__PURE__*/React.createElement("div",{className:"stats-grid",style:{marginBottom:20}},[{label:'Total',val:mensajeros.length,cls:''},{label:'Activos',val:mensajeros.filter(m=>m.activo).length,cls:'green'},{label:'Pausados',val:mensajeros.filter(m=>!m.activo).length,cls:'red'}].map(s=>/*#__PURE__*/React.createElement("div",{key:s.label,className:"stat-card"},/*#__PURE__*/React.createElement("div",{className:"stat-label"},s.label),/*#__PURE__*/React.createElement("div",{className:`stat-value ${s.cls}`},s.val)))),/*#__PURE__*/React.createElement("div",{className:"toolbar"},/*#__PURE__*/React.createElement("input",{className:"search-box",placeholder:"Buscar mensajero...",value:search,onChange:e=>setSearch(e.target.value)}),['todos','activos','pausados'].map(f=>/*#__PURE__*/React.createElement("button",{key:f,className:`filter-btn ${filtro===f?'active':''}`,onClick:()=>setFiltro(f)},f.charAt(0).toUpperCase()+f.slice(1)))),/*#__PURE__*/React.createElement("div",{className:"table-wrap"},/*#__PURE__*/React.createElement("table",null,/*#__PURE__*/React.createElement("thead",null,/*#__PURE__*/React.createElement("tr",null,/*#__PURE__*/React.createElement("th",null,"Mensajero"),esAdmin&&/*#__PURE__*/React.createElement("th",null,"Tarifa"),/*#__PURE__*/React.createElement("th",null,"Hoy: Total"),/*#__PURE__*/React.createElement("th",null,"Hoy: Entregas"),/*#__PURE__*/React.createElement("th",null,"Efectividad"),/*#__PURE__*/React.createElement("th",null,"Estado"),esAdmin&&/*#__PURE__*/React.createElement("th",null,"Ruta"),esAdmin&&/*#__PURE__*/React.createElement("th",null,"Ganancias"),esAdmin&&/*#__PURE__*/React.createElement("th",null,"Colecta"),/*#__PURE__*/React.createElement("th",null,"Acciones"))),/*#__PURE__*/React.createElement("tbody",null,filtrados.map(m=>{const s=statsMap[normNombre(m.nombre)];return/*#__PURE__*/React.createElement("tr",{key:m.id,className:m.activo?'':'paused'},/*#__PURE__*/React.createElement("td",{style:{fontWeight:600}},m.nombre,),esAdmin&&/*#__PURE__*/React.createElement("td",{className:"mono"},"$",fmt(m.tarifa)),/*#__PURE__*/React.createElement("td",{className:"mono"},s?s.total:'—'),/*#__PURE__*/React.createElement("td",{className:"mono",style:{color:'var(--success)'}},s?s.entregados:'—'),/*#__PURE__*/React.createElement("td",null,s?/*#__PURE__*/React.createElement(KpiBar,{value:s.ef}):/*#__PURE__*/React.createElement("span",{style:{color:'var(--text-soft)',fontSize:12}},"Sin datos")),/*#__PURE__*/React.createElement("td",null,/*#__PURE__*/React.createElement(Badge,{type:m.activo?'active':'paused'},m.activo?'● Activo':'● Pausado')),esAdmin&&/*#__PURE__*/React.createElement("td",null,/*#__PURE__*/React.createElement("button",{onClick:()=>toggleRutaIA(m.id),title:"Cobrable por mensajero",style:{display:'inline-flex',alignItems:'center',gap:6,padding:'4px 10px',borderRadius:20,border:'1px solid '+(m.optimizacionRutaActiva?'var(--success)':'var(--border)'),background:m.optimizacionRutaActiva?'rgba(46,125,79,0.1)':'#fff',cursor:'pointer',fontSize:11,fontWeight:700,color:m.optimizacionRutaActiva?'var(--success)':'var(--text-soft)'}},m.optimizacionRutaActiva?'● ON':'○ OFF')),esAdmin&&/*#__PURE__*/React.createElement("td",null,/*#__PURE__*/React.createElement("button",{onClick:()=>toggleVerGanancias(m.id),title:"Puede ver la pesta\xF1a Ganancias en su app",style:{display:'inline-flex',alignItems:'center',gap:6,padding:'4px 10px',borderRadius:20,border:'1px solid '+(m.verGanancias?'var(--success)':'var(--border)'),background:m.verGanancias?'rgba(46,125,79,0.1)':'#fff',cursor:'pointer',fontSize:11,fontWeight:700,color:m.verGanancias?'var(--success)':'var(--text-soft)'}},m.verGanancias?'● ON':'○ OFF')),esAdmin&&/*#__PURE__*/React.createElement("td",null,/*#__PURE__*/React.createElement("button",{onClick:()=>toggleColectaHabilitada(m.id),title:"Puede usar la pesta\xF1a Colecta en su app",style:{display:'inline-flex',alignItems:'center',gap:6,padding:'4px 10px',borderRadius:20,border:'1px solid '+(m.colectaHabilitada?'var(--success)':'var(--border)'),background:m.colectaHabilitada?'rgba(46,125,79,0.1)':'#fff',cursor:'pointer',fontSize:11,fontWeight:700,color:m.colectaHabilitada?'var(--success)':'var(--text-soft)'}},m.colectaHabilitada?'● ON':'○ OFF')),/*#__PURE__*/React.createElement("td",{style:{display:'flex',gap:5}},/*#__PURE__*/React.createElement("button",{className:"action-btn btn-edit",onClick:()=>{setForm({nombre:m.nombre,tarifa:m.tarifa,tarifaRetiro:m.tarifaRetiro||500,bloqueadoNombre:tieneUsuario(m)});setModal(m.id);}},"Editar"),esAdmin&&/*#__PURE__*/React.createElement("button",{style:{padding:'4px 10px',borderRadius:6,border:'1px solid rgba(200,168,75,0.4)',background:'transparent',color:'var(--gold)',cursor:'pointer',fontSize:11,fontWeight:700},onClick:()=>setNotifTarget(m)},"📢"),/*#__PURE__*/React.createElement("button",{className:`action-btn ${m.activo?'btn-pause':'btn-resume'}`,onClick:()=>toggleEstado(m.id)},m.activo?'Pausar':'Activar'),/*#__PURE__*/React.createElement("button",{className:"action-btn btn-delete",onClick:()=>setConfirmDel(m.id)},"\u2715")));}),filtrados.length===0&&/*#__PURE__*/React.createElement("tr",null,/*#__PURE__*/React.createElement("td",{colSpan:9,className:"empty-state"},"Sin resultados"))))),modal&&/*#__PURE__*/React.createElement(Modal,{title:modal==='add'?'Nuevo Mensajero':'Editar Mensajero',onClose:()=>setModal(null)},
  ...
  })),confirmDel&&/*#__PURE__*/React.createElement(Modal,{title:"\xBFEliminar mensajero?",onClose:()=>setConfirmDel(null)},/*#__PURE__*/React.createElement("p",{style:{fontSize:13,color:'var(--text-mid)',marginBottom:24}},"Esta acci\xF3n no se puede deshacer. \xBFConfirmas eliminar este mensajero del sistema?"),/*#__PURE__*/React.createElement("div",{className:"modal-actions"},/*#__PURE__*/React.createElement("button",{className:"btn-secondary",onClick:()=>setConfirmDel(null)},"Cancelar"),/*#__PURE__*/React.createElement("button",{className:"btn-primary",style:{background:'var(--danger)',borderColor:'var(--danger)',color:'#fff'},onClick:()=>del(confirmDel)},"Eliminar"))),grupoDuplicados&&/*#__PURE__*/React.createElement(Modal,{title:`${grupoDuplicados.length} nombre${grupoDuplicados.length>1?'s':''} duplicado${grupoDuplicados.length>1?'s':''}`,sub:"Elige qu\xE9 fila mantener en cada grupo. Las dem\xE1s se eliminan de Supabase (no se puede deshacer).",onClose:()=>setGrupoDuplicados(null),wide:true},grupoDuplicados.map(g=>/*#__PURE__*/React.createElement("div",{key:g.clave,style:{marginBottom:18,padding:14,background:'var(--cream)',borderRadius:8,border:'1px solid var(--border)'}},
