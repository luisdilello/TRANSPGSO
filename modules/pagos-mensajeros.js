...
useEffect(()=>{
  ...
  if(mensajeros.length>0&&pagos.length>0){
    const tarifaMap={};
    mensajeros.forEach(m=>{tarifaMap[m.nombre]=m.tarifa||1200;});
    setPagos(prev=>prev.map(p=>{
      const tarifaActual=tarifaMap[p.nombre]||p.tarifa||1200;
