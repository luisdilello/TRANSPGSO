  ...
  if(saved.length>0){
    ...
    const updated=savedActivos.map(p=>{
      ...
      if(menActual){
        ...
        const totalPagar=totalBruto+(p.extra||0)-(p.adelanto||0)-(p.prestamo||0)-(p.consumo||0)-(p.descSiniestro||0);
...
useEffect(()=>{
  ...
  db.from('siniestros').select('mensajero,descontado_mensajero_valor').eq('descontado_mensajero_semana',semana).eq('descontado_mensajero',true).then(function(r){
    ...
    setPagos(prev=>prev.map(p=>{
      ...
      updated.totalPagar=updated.totalBruto+(updated.extra||0)-(updated.adelanto||0)-(updated.prestamo||0)-(updated.consumo||0)-(updated.descSiniestro||0);
      return updated;
    }));
  ...
  try{
    ...
    setPagos(function(prev){
      ...
      return prev.map(function(p){
        ...
        var totalPagar=totalBruto+(p.extra||0)-(p.adelanto||0)-(p.prestamo||0)-(p.consumo||0)-(p.descSiniestro||0);
