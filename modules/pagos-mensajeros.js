...
function PagosTarjetas(props){
  var pagos=props.pagos, montoPago=props.montoPago, updatePago=props.updatePago,
      ...
      excluirDescuentoEfectividad=props.excluirDescuentoEfectividad,
      esAdmin=props.esAdmin, eliminarPenalizacion=props.eliminarPenalizacion;

  // Qué tarjetas tienen el desglose de Descuento por Efectividad desplegado -- Luis pidió poder
  // abrir el detalle completo (fecha + código de cada paquete) en vez de depender solo del
  // tooltip al pasar el mouse, con una X para excluir un paquete puntual si el descuento no
  // corresponde.
  var _useExpandEfect=useState({}),expandidoEfect=_useExpandEfect[0],setExpandidoEfect=_useExpandEfect[1];
  // Mismo desplegable para Penalización -- Luis: "asi como puedo desplegar y ver y eliminar la
  // penalizacion de baja efectividad pueda hacer lo mismo con [el paquete] reportado por algun
  // operador o supervisor". El botón ✕ para borrar cada multa solo se muestra si esAdmin=true
  // ("que solo yo pueda borrar") -- un operador con acceso de solo vista a Pagos Mensajeros (vía
  // el permiso 'pagos' en Permisos) puede ver el detalle pero no eliminar nada desde acá.
  var _useExpandPenal=useState({}),expandidoPenal=_useExpandPenal[0],setExpandidoPenal=_useExpandPenal[1];
    ...
    pagos.map(function(p){
      ...
      return React.createElement('div',{key:p.id,className:'stat-card',style:{'--card-accent':pagado?'#2e7d4f':'#C8A84B',borderTop:'4px solid '+(pagado?'#2e7d4f':'#C8A84B'),padding:'18px 18px 14px'}},
        ...
        React.createElement('div',{style:{marginBottom:10}},
          ...
          fila('Siniestro',React.createElement('span',{title:(p.detalleSiniestro&&p.detalleSiniestro.length)?('Código(s): '+p.detalleSiniestro.map(function(d){return d.codigo;}).join(', ')):'Sin descuentos por siniestro esta semana'},'$'+Math.round(p.descSiniestro||0).toLocaleString('es-CL')),'#C62828'),
          ...
          (p.descuentoEfectividad||0)>0?fila('Desc. Efect.',React.createElement('span',{
            style:{cursor:(p.detalleEfectividad&&p.detalleEfectividad.length)?'pointer':'default'},
            title:(p.detalleEfectividad&&p.detalleEfectividad.length)?'Clic para ver el detalle completo':'',
            onClick:function(){if(!(p.detalleEfectividad&&p.detalleEfectividad.length))return;setExpandidoEfect(function(prev){return Object.assign({},prev,{[p.id]:!prev[p.id]});});}
          },'$'+Math.round(p.descuentoEfectividad).toLocaleString('es-CL')+' ('+(p.paquetesNoEntregadosEfectividad||0)+' paq. en '+(p.diasBajoEfectividad||0)+' día'+(p.diasBajoEfectividad===1?'':'s')+')'+((p.detalleEfectividad&&p.detalleEfectividad.length)?(expandidoEfect[p.id]?' ▲':' ▼'):'')),'var(--danger)'):null,
          (expandidoEfect[p.id]&&p.detalleEfectividad&&p.detalleEfectividad.length)?React.createElement('div',{key:'detEfect-'+p.id,style:{background:'rgba(176,48,48,0.06)',border:'1px solid rgba(176,48,48,0.25)',borderRadius:8,padding:'8px 10px',margin:'2px 0 6px'}},
            p.detalleEfectividad.map(function(d,di){
              return React.createElement('div',{key:d.fecha+'-'+di,style:{marginBottom:di<p.detalleEfectividad.length-1?8:0,paddingBottom:di<p.detalleEfectividad.length-1?8:0,borderBottom:di<p.detalleEfectividad.length-1?'1px solid rgba(176,48,48,0.15)':'none'}},
                React.createElement('div',{style:{fontSize:10,fontWeight:800,color:'var(--text-main)',marginBottom:4,textTransform:'uppercase',letterSpacing:0.3}},
                  new Date(d.fecha+'T12:00:00').toLocaleDateString('es-CL')+' · '+d.paquetes+' paq. sin entregar (bajo el umbral de efectividad ese día)'
                ),
                (d.codigos&&d.codigos.length?d.codigos:[]).map(function(cod){
                  return React.createElement('div',{key:cod,style:{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'3px 0'}},
                    React.createElement('span',{style:{fontFamily:'JetBrains Mono',fontSize:11}},cod),
                    React.createElement('button',{
                      onClick:function(){excluirDescuentoEfectividad(p.id,d.fecha,cod);},
                      title:'Excluir este paquete del descuento por efectividad',
                      style:{border:'none',background:'transparent',color:'var(--danger)',cursor:'pointer',fontWeight:800,fontSize:13,padding:'0 6px',lineHeight:1}
                    },'✕')
                  );
                }),
                (!d.codigos||!d.codigos.length)?React.createElement('div',{style:{fontSize:11,color:'var(--text-soft)',fontStyle:'italic'}},'Sin código registrado para este día'):null
              );
            })
          ):null,
          // Mismo tratamiento que Desc. Efectividad: clic para desplegar el detalle completo de
          // cada multa (código del paquete si vino de "⚠ Reportar" en Firmas en Vivo, motivo,
          // fecha y nota) en vez de depender solo del tooltip -- Luis: "es demasiado importante
          // que esto se detalle... el código, el motivo, la fecha". El botón ✕ para eliminar cada
          // multa solo aparece si esAdmin=true ("que solo yo pueda borrar").
          (p.penalizacion||0)>0?fila('Penalización',React.createElement('span',{
            style:{cursor:(p.penalizaciones&&p.penalizaciones.length)?'pointer':'default'},
            title:(p.penalizaciones&&p.penalizaciones.length)?'Clic para ver el detalle completo':'',
            onClick:function(){if(!(p.penalizaciones&&p.penalizaciones.length))return;setExpandidoPenal(function(prev){return Object.assign({},prev,{[p.id]:!prev[p.id]});});}
          },'-'+(p.penalizaciones||[]).reduce(function(a,x){return a+(x.envios||0);},0)+' env. ($'+Math.round(p.penalizacion).toLocaleString('es-CL')+')'+((p.penalizaciones&&p.penalizaciones.length)?(expandidoPenal[p.id]?' ▲':' ▼'):'')),'var(--danger)'):null,
          (expandidoPenal[p.id]&&p.penalizaciones&&p.penalizaciones.length)?React.createElement('div',{key:'detPenal-'+p.id,style:{background:'rgba(176,48,48,0.06)',border:'1px solid rgba(176,48,48,0.25)',borderRadius:8,padding:'8px 10px',margin:'2px 0 6px'}},
            p.penalizaciones.map(function(x,xi){
              const fechaFmt=x.fecha?new Date(x.fecha+'T12:00:00').toLocaleDateString('es-CL'):'sin fecha';
              return React.createElement('div',{key:x.id||xi,style:{marginBottom:xi<p.penalizaciones.length-1?8:0,paddingBottom:xi<p.penalizaciones.length-1?8:0,borderBottom:xi<p.penalizaciones.length-1?'1px solid rgba(176,48,48,0.15)':'none'}},
                React.createElement('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8}},
                  React.createElement('div',{style:{minWidth:0}},
                    React.createElement('div',{style:{fontSize:11,fontWeight:800,color:'var(--text-main)'}},x.motivo),
                    React.createElement('div',{style:{fontSize:10,color:'var(--text-soft)',marginTop:2}},fechaFmt+' · '+(x.envios||0)+' env. · $'+Math.round(x.monto||0).toLocaleString('es-CL')+(x.nota?' · '+x.nota:'')),
                    x.codigo?React.createElement('div',{style:{fontFamily:'JetBrains Mono',fontSize:11,marginTop:3}},x.codigo):React.createElement('div',{style:{fontSize:10,color:'var(--text-soft)',fontStyle:'italic',marginTop:3}},'Sin código de envío asociado')
                  ),
                  esAdmin?React.createElement('button',{
                    onClick:function(){
                      if(!window.confirm('¿Eliminar esta multa de '+p.nombre.replace(/,\s*/g,' ')+'?\n\n'+x.motivo+(x.codigo?' · Código '+x.codigo:'')+'\n$'+Math.round(x.monto||0).toLocaleString('es-CL')+'\n\nEl monto se va a sumar de vuelta al Total a Pagar.'))return;
                      eliminarPenalizacion(p.id,x.id);
                    },
                    title:'Eliminar esta multa',
                    style:{border:'none',background:'transparent',color:'var(--danger)',cursor:'pointer',fontWeight:800,fontSize:13,padding:'0 6px',lineHeight:1,flexShrink:0}
                  },'✕'):null
                )
              );
            })
...
// Documento de UN cliente puntual dentro de "Cobros a Clientes" -- Luis pidió que el botón
// "Detalle de Cobro" de la tabla "Resumen por Cliente" muestre EXACTAMENTE el mismo documento
// que ya arma Calendario de Cobros (el botón "📄 Recibo"), solo que con el rótulo "Detalle de
// Cobro" en vez de "Recibo de Cobro". La primera versión de este botón armaba su propio HTML
// simplificado a mano (con menos secciones y sin los datos bancarios) y además abría un
// window.print() automático -- Luis lo reportó: "funciona bien desde el dashboard pero no
// desde cobro a clientes... suelta un pdf que no es el mismo". Ahora este botón reutiliza
// generarReciboCobro (el mismo generador de index.html, expuesto vía window.__app) pasándole
// 'Detalle de Cobro' como título, así el cálculo, el diseño y los gráficos quedan
// idénticos al Recibo oficial, y no dispara ningún print automático.
...
function CobrosClientesTab(_ref4){
  ...
  // "📋 Detalle de Cobro" / "⬇ Descargar" por fila -- reutiliza generarReciboCobro (el MISMO
  // generador que usa el botón "📄 Recibo" de Calendario de Cobros, expuesto vía window.__app),
  // pasándole 'Detalle de Cobro' como título. Así el cálculo, el diseño y los gráficos son
  // idénticos al Recibo oficial (antes esta pestaña armaba su propio HTML simplificado y
  // terminaba mostrando un documento distinto, con menos datos, y con un print automático que
  // el Recibo real no tiene -- por eso Luis reportó "suelta un pdf que no es el mismo").
  ...
  function abrirDetalleCobro(r){
    ...
    generarReciboCobro(r.nombre,enviosDeCliente(r),periodo.label,cliData,undefined,undefined,undefined,'Detalle de Cobro');
  }
...
// Mensajeros inactivos: NO deben seguir apareciendo en Pagos (tabla, tarjetas ni TOTALES) una vez
// desactivados. 'pagos' solo filtraba por mensajero activo al CREAR una fila nueva (import diario,
// "Calcular Envíos Semana", sincronización de mensajerosDia) -- pero esas filas se guardan en el
// arreglo 'pagos' (initializer de useState que corre una sola vez al montar) y ningún efecto las
// vuelve a sacar si el mensajero se desactiva DESPUÉS de que su fila ya existía. Resultado: un
// mensajero dado de baja a mitad de semana se quedaba pegado en la vista con $0 indefinidamente.
// Se filtra acá, a nivel de visualización -- no se borra la fila de 'pagos' ni de localStorage,
// así que si el admin lo reactiva por error, su fila (y lo que llevaba acumulado) reaparece tal
// cual, sin recalcular nada.
const nombresActivosSet=React.useMemo(()=>new Set(mensajeros.filter(m=>m.activo!==false).map(m=>m.nombre.replace(/,\s*/g,' ').toUpperCase().trim())),[mensajeros]);
function mensajeroActivo(p){return nombresActivosSet.has((p.nombre||'').replace(/,\s*/g,' ').toUpperCase().trim());}
...
// sumando TODOS los pagos VISIBLES (de mensajeros activos) de la semana, filtrados por búsqueda
// o no, para que nunca se lea como si el total mostrado fuera parcial.
...
const pagosMostrados=React.useMemo(()=>{
  let arr=pagos.filter(mensajeroActivo);
  ...
  return arr;
},[pagos,pagosBusqueda,pagosOrden,nombresActivosSet]);
...
// Además de la suma, se guarda el detalle código por código (p.detalleSiniestro) -- Luis pidió
// poder ver EXACTAMENTE qué paquete(s) generaron el descuento en el Detalle de Pago, no solo
// el monto total.
useEffect(()=>{
  if(!_pagosCargados.current)return;
  const normNomSin=n=>(n||'').replace(/,\s*/g,' ').replace(/\s+/g,' ').trim().toUpperCase();
  db.from('siniestros').select('codigo,mensajero,descontado_mensajero_valor,fecha_siniestro').eq('descontado_mensajero_semana',semana).eq('descontado_mensajero',true).then(function(r){
    if(r.error){console.warn('Error cargando descuentos de siniestro:',r.error.message);return;}
    const sumaPorMensajero={};
    const detallePorMensajero={};
    (r.data||[]).forEach(function(s){
      ...
      if(!detallePorMensajero[key])detallePorMensajero[key]=[];
      detallePorMensajero[key].push({codigo:s.codigo,valor:parseFloat(s.descontado_mensajero_valor)||0,fecha:s.fecha_siniestro||''});
    });
    setPagos(prev=>prev.map(p=>{
      const key=normNomSin(p.nombre);
      const nuevoDesc=sumaPorMensajero[key]||0;
      const nuevoDetalle=detallePorMensajero[key]||[];
      if((p.descSiniestro||0)===nuevoDesc&&(p.detalleSiniestro||[]).length===nuevoDetalle.length)return p;
      const updated={...p,descSiniestro:nuevoDesc,detalleSiniestro:nuevoDetalle};
...
},[mensajeros,pagos.length]);useEffect(()=>{if(mensajerosDia.length>0){const normNom2=n=>(n||'').replace(/,\s*/g,' ').replace(/\s+/g,' ').trim().toUpperCase();const tarifaMap={};mensajeros.forEach(m=>{tarifaMap[normNom2(m.nombre)]=m.tarifa||1200;});setPagos(prev=>{const prevMap={};prev.forEach(p=>{prevMap[normNom2(p.nombre)]=p;});return mensajerosDia.filter(m=>m.total>0).map((m,i)=>{const tarifa=tarifaMap[normNom2(m.nombre)]||1200;const bruto=m.entregados*tarifa;const existing=prevMap[normNom2(m.nombre)];if(existing)return{...existing,envios:m.entregados};return{id:m.id||i,nombre:m.nombre,envios:m.entregados,tarifa,bruto,ajuste:0,iva:0,totalBruto:bruto,adelanto:0,extra:0,prestamo:0,consumo:0,descSiniestro:0,penalizacion:0,totalPagar:bruto,estado:'PENDIENTE',obs:''};});});}},[mensajerosDia]);function updatePago(id,field,val){setPagos(prev=>prev.map(p=>{if(p.id!==id)return p;const strFields=['estado','obs','tipoIVA','prestamoDescontado','adelantoDescontado'];const updated={...p,[field]:strFields.includes(field)?val:+val};if(field==='tipoIVA'){if(val==='ninguno'){updated.iva=0;}else if(val==='manual'){}else{const rate=val==='honorarios'?0.1525:val==='factura'?0.19:0;updated.iva=Math.round(updated.bruto*rate/(1+rate));}}if(field==='tarifa'){updated.bruto=updated.envios*+val;const rate=updated.tipoIVA==='honorarios'?0.1525:updated.tipoIVA==='factura'?0.19:0;if(rate>0)updated.iva=Math.round(updated.bruto*rate/(1+rate));}if(!strFields.includes(field)){updated.totalBruto=updated.bruto+updated.ajuste-updated.iva;updated.totalPagar=updated.totalBruto+updated.extra+(updated.bonoEfectividad||0)-(updated.descuentoEfectividad||0)-updated.adelanto-updated.prestamo-updated.consumo-(updated.descSiniestro||0)-(updated.penalizacion||0);}if(field==='tipoIVA'){updated.totalBruto=updated.bruto+updated.ajuste-updated.iva;updated.totalPagar=updated.totalBruto+updated.extra+(updated.bonoEfectividad||0)-(updated.descuentoEfectividad||0)-updated.adelanto-updated.prestamo-updated.consumo-(updated.descSiniestro||0)-(updated.penalizacion||0);}return updated;}));}
// Luis pidió, sobre el desglose de Descuento por Efectividad de la tarjeta de pago, poder
// "excluir" un paquete puntual si el descuento de ese código no corresponde (ej. el mensajero
// tiene un justificativo). Al excluirlo: (1) se resta ESE paquete del detalle y del monto
// descontado de ESTE pago -- ya, sin esperar a un recálculo -- y (2) se guarda el código
// excluido en p.excluidosEfectividad para que la próxima vez que corra "Calcular Envíos
// Semana" (incluso automático al recargar la página) NO vuelva a aparecer, ya que ese cálculo
// recorre todo de nuevo desde 'envios'/'historial_envios' y si no tuviera en cuenta esta
// lista, el paquete excluido reaparecería con el próximo recálculo.
function excluirDescuentoEfectividad(pagoId,fecha,codigo){
  setPagos(function(prev){
    return prev.map(function(p){
      if(p.id!==pagoId)return p;
      if(!window.confirm('¿Excluir el paquete '+codigo+' ('+new Date(fecha+'T12:00:00').toLocaleDateString('es-CL')+') del descuento por efectividad de '+p.nombre.replace(/,\s*/g,' ')+'?\n\nEl monto de ese paquete se va a sumar de vuelta al Total a Pagar.'))return p;
      var excluidosPrevios=p.excluidosEfectividad||[];
      var yaExcluido=excluidosPrevios.some(function(x){return x.fecha===fecha&&x.codigo===codigo;});
      var nuevosExcluidos=yaExcluido?excluidosPrevios:excluidosPrevios.concat([{fecha:fecha,codigo:codigo}]);
      var nuevoDetalle=(p.detalleEfectividad||[]).map(function(d){
        if(d.fecha!==fecha)return d;
        var teniaCodigo=(d.codigos||[]).indexOf(codigo)!==-1;
        var nuevosCodigos=(d.codigos||[]).filter(function(c){return c!==codigo;});
        return Object.assign({},d,{codigos:nuevosCodigos,paquetes:Math.max((d.paquetes||0)-(teniaCodigo?1:0),0)});
      }).filter(function(d){return (d.paquetes||0)>0;});
      var nuevoTotalPaquetes=nuevoDetalle.reduce(function(a,d){return a+(d.paquetes||0);},0);
      var nuevoDescuento=nuevoTotalPaquetes*(+criterioEf.descuento||0);
      var updated=Object.assign({},p,{excluidosEfectividad:nuevosExcluidos,detalleEfectividad:nuevoDetalle,paquetesNoEntregadosEfectividad:nuevoTotalPaquetes,diasBajoEfectividad:nuevoDetalle.length,descuentoEfectividad:nuevoDescuento});
      updated.totalPagar=updated.totalBruto+(updated.extra||0)+(updated.bonoEfectividad||0)-nuevoDescuento-(updated.adelanto||0)-(updated.prestamo||0)-(updated.consumo||0)-(updated.descSiniestro||0)-(updated.penalizacion||0);
      if(toast)toast('✓ Paquete '+codigo+' excluido del descuento por efectividad');
      return updated;
    });
  });
}
function aplicarPenalizacion(pagoId,item){
  setPagos(function(prev){
    return prev.map(function(p){
      if(p.id!==pagoId)return p;
...
async function calcularEnviosSemana(){
  ...
  try{
    ...
    var todosAsignados=(criterioEf.bonoActivo||criterioEf.descuentoActivo)?await fetchPaginadoParalelo(function(cursor,limite){
      return db.from('envios').select('id,codigo,mensajero,estado,fecha')
    ...
    // Además del conteo, se guarda el CÓDIGO de cada paquete asignado/entregado ese día
    // (asignadosCodigosPorDia/entregadosCodigosPorDia) -- así, cuando un día queda bajo el
    // umbral, más abajo se arma la lista EXACTA de qué código(s) no se entregaron ese día
    // (Luis pidió poder ver qué paquete específico penalizó, no solo un número).
    var asignadosPorDia={};
    var asignadosCodigosPorDia={};
    todosAsignados.forEach(function(e){
      var n=normNombre(e.mensajero||'');
      if(!n||n==='SIN ASIGNAR'||n==='')return;
      var f=e.fecha;
      ...
      if(!asignadosCodigosPorDia[n])asignadosCodigosPorDia[n]={};
      if(!asignadosCodigosPorDia[n][f])asignadosCodigosPorDia[n][f]=[];
      if(e.codigo)asignadosCodigosPorDia[n][f].push(e.codigo);
    });
    var entregadosPorDia={};
    var entregadosCodigosPorDia={};
    data.forEach(function(e){
      var n=normNombre(e.mensajero||'');
      if(!n||n==='SIN ASIGNAR'||n==='')return;
      ...
      if(!entregadosCodigosPorDia[n])entregadosCodigosPorDia[n]={};
      if(!entregadosCodigosPorDia[n][f])entregadosCodigosPorDia[n][f]=new Set();
    ...
    setPagos(function(prev){
      ...
      return prev.map(function(p){
        ...
        // auto-cálculo al recargar la página) no los vuelva a traer -- si no se respetara esta
        // lista, la exclusión manual se perdería en cuanto se recalculara de nuevo.
        var excluidosSet=new Set((p.excluidosEfectividad||[]).map(function(x){return x.fecha+'|'+x.codigo;}));
        var diasBajoEfectividad=0,paquetesNoEntregadosEfectividad=0,detalleEfectividad=[];
        if(criterioEf.descuentoActivo){
          var asigDia=asignadosPorDia[key]||{};
          var entDia=entregadosPorDia[key]||{};
          var asigCodDia=asignadosCodigosPorDia[key]||{};
          var entCodDia=entregadosCodigosPorDia[key]||{};
          Object.keys(asigDia).sort().forEach(function(f){
            ...
            if((efecD*100)<(+criterioEf.umbralDescuentoPct||0)){
              ...
              if(faltantes<=0)return; // se excluyeron todos los paquetes de este día -- no cuenta como incidente
              diasBajoEfectividad++;
              paquetesNoEntregadosEfectividad+=faltantes;
              detalleEfectividad.push({fecha:f,paquetes:faltantes,codigos:codigosFaltantes});
            }
          });
        }
...
const totales=pagos.filter(mensajeroActivo).reduce((a,p)=>{const m=montoPago(p);return{envios:a.envios+p.envios,bruto:a.bruto+p.bruto,adelanto:a.adelanto+p.adelanto,extra:a.extra+p.extra,bonoEfectividad:a.bonoEfectividad+m.bono,descuentoEfectividad:a.descuentoEfectividad+m.desc,prestamo:a.prestamo+p.prestamo,iva:a.iva+p.iva,consumo:a.consumo+(p.consumo||0),descSiniestro:a.descSiniestro+(p.descSiniestro||0),total:a.total+m.total,pendientes:a.pendientes+(p.estado==='PENDIENTE'?1:0),pagados:a.pagados+(p.estado==='PAGADO'?1:0)};},{envios:0,bruto:0,adelanto:0,extra:0,bonoEfectividad:0,descuentoEfectividad:0,prestamo:0,iva:0,consumo:0,descSiniestro:0,total:0,pendientes:0,pagados:0});const fmtCLP=n=>`$${Math.round(n).toLocaleString('es-CL')}`;function exportarComprobante(p){var _document$querySelect5;const win=window.open('','_blank','width=650,height=860');const logoSrc=((_document$querySelect5=document.querySelector('.logo-img'))==null?void 0:_document$querySelect5.src)||'';
    ...
    <title>Detalle de Pago - ${p.nombre.replace(/,\s*/g,' ')}</title>
    <style>
      ...
      <div class="pad">
        <div class="header">
          ...
          <div style="text-align:right">
            <div class="titulo">Detalle de Pago</div>
        ...
        <div class="footer-note">Detalle de pago generado automáticamente por TransPgso · ${new Date().toLocaleString('es-CL')}</div>
      </div>
    </div>
