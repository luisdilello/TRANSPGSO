...
const pdfRef=useRef();const _usePS=useState(50),PAGE_SIZE=_usePS[0],setPageSize=_usePS[1];const fileRef=useRef();useEffect(()=>{lsSave('gestion_envios',envios);},[envios]);useEffect(()=>{sincronizarDesdeSupabase();const _autoSyncInterval=setInterval(sincronizarDesdeSupabase,60000);return()=>clearInterval(_autoSyncInterval);},[]);async function sincronizarDesdeSupabase(){setSincronizando(true);try{
