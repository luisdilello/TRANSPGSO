...
const _useGestionGeoCfg=useState({activo:false,radio_metros:150,estados:['reprogramado','cancelado']}),gestionGeoCfg=_useGestionGeoCfg[0],setGestionGeoCfg=_useGestionGeoCfg[1];
...
useEffect(function(){
  (async function(){
    try{
      ...
      if(data&&data.valor&&typeof data.valor==='object')setGestionGeoCfg({activo:!!data.valor.activo,radio_metros:data.valor.radio_metros||150,estados:Array.isArray(data.valor.estados)?data.valor.estados:['reprogramado','cancelado']});
    }catch(e){}
...
async function cambiarEstado(ids,nuevoEstado,nota){if(nota===void 0){nota='';}const enviosAfectados=envios.filter(e=>ids.has(e.id));setEnvios(prev=>prev.map(e=>{if(!ids.has(e.id))return e;const notaFinal=nota||`Estado cambiado a ${estadoInfo(nuevoEstado).label}`+(e.mensajero?` (mensajero: ${e.mensajero.replace(/,\s*/g,' ')})`:'');return{...e,estado:nuevoEstado,mensajero:nuevoEstado==='sin_asignar'?'':e.mensajero,historial:[...e.historial,crearEntradaHistorial(nuevoEstado,notaFinal,usuario?.nombre||'Admin')]};}));setSelected(new Set());toast(`✓ ${ids.size} envío${ids.size>1?'s':''} → ${estadoInfo(nuevoEstado).label}`);for(const e of enviosAfectados){try{await db.from('envios').upsert({codigo:e.codigo,cliente:e.cliente||'',destinatario:e.destinatario||'',telefono:e.telefono||'',direccion:e.direccion||'',comuna:e.comuna||'',referencia:e.referencia||'',fecha:e.fecha||fechaHoyCL(),estado:nuevoEstado,mensajero:nuevoEstado==='sin_asignar'?'':e.mensajero,monto:e.monto||0,en_un_cambio:e.enUnCambio||false,nota:nota||e.nota||'',fuente:e.fuente||'propio'},{onConflict:'codigo'});sbRegistrarHistorial(e.codigo,nuevoEstado,nota||`Estado cambiado a ${estadoInfo(nuevoEstado).label}`,usuario?.nombre||'Admin','panel_admin');if(nuevoEstado==='siniestro'&&window.__app&&window.__app.registrarSiniestro){try{await window.__app.registrarSiniestro(e.codigo,e.mensajero||'',nota||'');}catch(errSin){console.warn('registrarSiniestro error:',errSin.message);}}}catch(err){console.warn('Supabase sync error:',err.message);}}}async function asignarMensajero(){if(!mensajeroAsignar){toast('Selecciona un mensajero');return;}const ids=selected;const enviosSeleccionados=envios.filter(e=>ids.has(e.id));enviosSeleccionados.forEach(e=>{edicionesRecientesRef.current[e.codigo]={estado:'en_ruta',mensajero:mensajeroAsignar,ts:Date.now()};});setEnvios(prev=>prev.map(e=>{if(!ids.has(e.id))return e;return{...e,mensajero:mensajeroAsignar,estado:'en_ruta',historial:[...e.historial,crearEntradaHistorial('en_ruta',`Asignado a ${mensajeroAsignar}`,usuario?.nombre||'Admin')]};}));for(const e of enviosSeleccionados){try{await db.from('envios').upsert({codigo:e.codigo,cliente:e.cliente||'',destinatario:e.destinatario||'',telefono:e.telefono||'',direccion:e.direccion||'',comuna:e.comuna||'',referencia:e.referencia||'',fecha:e.fecha||fechaHoyCL(),estado:'en_ruta',mensajero:mensajeroAsignar,monto:e.monto||0,en_un_cambio:e.enUnCambio||false,nota:e.nota||'',fuente:e.fuente||'propio'},{onConflict:'codigo'});sbRegistrarHistorial(e.codigo,'en_ruta','Asignado a '+mensajeroAsignar+' desde panel admin',usuario?.nombre||'Admin','panel_admin');}catch(err){console.warn('Supabase sync error:',err.message);}}setSelected(new Set());setAsignarModal(false);playSound('ruta');toast(`✓ ${ids.size} envío${ids.size>1?'s':''} asignado${ids.size>1?'s':''} a ${mensajeroAsignar} · Sincronizado`);}async function eliminarSeleccionados(){
...
const filtrados=useMemo(()=>{const q=search.trim().toLowerCase();
  ...
  return baseLista.filter(e=>{const qTerms=q.split(/[\n,;\s]+/).map(t=>t.trim().toLowerCase()).filter(Boolean);
      ...
      const matchQ=!q||(esMultiple
        ...
        :e.codigo.toLowerCase().includes(q)||e.destinatario.toLowerCase().includes(q)||e.direccion.toLowerCase().includes(q)||e.comuna.toLowerCase().includes(q)||e.cliente.toLowerCase().includes(q)||(e.mensajero||'').toLowerCase().includes(q)||estadoInfo(e._estadoEfectivo||e.estado).label.toLowerCase().includes(q)||(e.fecha||'').toLowerCase().includes(q));const matchEst=filtroEst==='todos'||filtroEst==='entregado'||filtroEst==='retorno'||(e._estadoEfectivo||e.estado)===filtroEst;const matchCli=filtroCli==='todos'||e.cliente===filtroCli;const matchMen=filtroMen==='todos'||e.mensajero===filtroMen;const matchFuente=filtroFuente==='todos'||e.fuente===filtroFuente;const matchAtraso=filtroAtrasoModo==='off'?true:filtroAtrasoModo==='atrasados'?esEnvioAtrasado(e):filtroAtrasoModo==='reprogramados'?esReprogramadoRepetido(e):(esEnvioAtrasado(e)||esReprogramadoRepetido(e));return matchQ&&matchEst&&matchCli&&matchMen&&matchFuente&&matchAtraso;});},[envios,entregadosPeriodoReal,retornadosPeriodoReal,todosPeriodoReal,enviosPeriodoEfectivo,search,filtroEst,filtroCli,filtroMen,filtroFuente,filtroAtrasoModo,reprogCount]);
...
const atrasadosDetalleFiltrados=useMemo(()=>{
  ...
  const baseLista=filtroEst==='entregado'?entregadosPeriodoReal:filtroEst==='retorno'?retornadosPeriodoReal:enviosPeriodoEfectivo;
  return baseLista.filter(e=>{
    ...
    const matchEst=filtroEst==='todos'||filtroEst==='entregado'||filtroEst==='retorno'||(e._estadoEfectivo||e.estado)===filtroEst;
...
},[enviosPeriodo,entregadosPeriodoReal,retornadosPeriodoReal,enviosPeriodoEfectivo,filtroEst,search,filtroCli,filtroMen,filtroFuente]);
...
/*#__PURE__*/React.createElement("div",{style:{marginBottom:16}},
  /*#__PURE__*/React.createElement("div",{style:{fontFamily:'Bebas Neue',fontSize:14,letterSpacing:1.5,color:'var(--dark)',marginBottom:8}},"Se exige en estos estados"),
  ['reprogramado','cancelado'].map(function(est){
    return /*#__PURE__*/React.createElement("label",{key:est,style:{display:'flex',alignItems:'center',gap:8,fontSize:13,marginBottom:6,cursor:'pointer'}},
      /*#__PURE__*/React.createElement("input",{type:'checkbox',checked:gestionGeoCfg.estados.includes(est),onChange:()=>setGestionGeoCfg(prev=>Object.assign({},prev,{estados:prev.estados.includes(est)?prev.estados.filter(function(x){return x!==est;}):prev.estados.concat([est])}))}),
      est==='reprogramado'?'Reprogramado':'Cancelado'
    );
  })
),
