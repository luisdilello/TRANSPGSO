...
/*#__PURE__*/React.createElement('div',{style:{display:'flex',gap:8,alignItems:'center',marginBottom:14,flexWrap:'wrap',paddingTop:10}},
  ...
  /*#__PURE__*/React.createElement('div',{style:{marginLeft:'auto',display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}},
    ...
    // NUEVO 2026-09-23: tercer badge con el resultado YA calculado -- Luis pidió "una vista con
    // esta lógica" en vez de tener que restar los dos números de arriba a mano cada vez. Se pone
    // acá (fuera del selector de VISTA, junto a los otros dos) para que se vea SIEMPRE, sin
    // importar en qué vista esté parado -- es la fórmula de cobro (recibido − retorno) ya resuelta.
    /*#__PURE__*/React.createElement('span',{title:'Total a pagar/facturar = recibidos en el período MENOS retorno resuelto en el período. Ya calculado -- no hace falta restar a mano ni cambiar de vista.',style:{display:'flex',alignItems:'center',gap:8,fontFamily:'Bebas Neue',fontSize:18,letterSpacing:1,color:'#fff',background:'linear-gradient(145deg,#3a5a40,#2e4632)',border:'1.5px solid #2e4632',borderRadius:12,padding:'6px 16px',boxShadow:'3px 3px 8px rgba(43,46,32,0.15)'}},
      (sincronizando||cargandoRetornadosReal)?/*#__PURE__*/React.createElement('span',{style:{fontSize:13,fontFamily:'DM Sans',color:'#fff'}},'Calculando...'):/*#__PURE__*/React.createElement(React.Fragment,null,
        /*#__PURE__*/React.createElement('span',{style:{color:'#fff',fontSize:22}},(enviosPeriodoTarjetas.length-retornadosPeriodoRealTarjetas.length).toLocaleString('es-CL')),
        /*#__PURE__*/React.createElement('span',{style:{fontSize:11,fontFamily:'DM Sans',color:'rgba(255,255,255,0.85)',letterSpacing:0,textTransform:'none'}},'TOTAL A PAGAR (recibido − retorno)')
      )
    )
  )
),
...
// NUEVO 2026-09-23: hoja "Resumen" con la fórmula de cobro ya calculada (recibido − retorno),
// para que el número que se le manda al cliente quede impreso en el mismo Excel y no haya que
// restarlo a mano ni explicar de nuevo de dónde salió.
const totalRecibidos=enviosPeriodoTarjetas.length;
const totalRetornoResuelto=retornadosPeriodoRealTarjetas.length;
sheets.push({name:'Resumen',headers:['Concepto','Cantidad'],rows:[
  ['Recibidos en el período',totalRecibidos],
  ['Retorno resuelto en el período',totalRetornoResuelto],
  ['TOTAL A PAGAR (recibido − retorno)',totalRecibidos-totalRetornoResuelto]
]});
